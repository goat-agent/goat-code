...
async fn load_code_assist(
    ...
    if !status.is_success() {
        return Err(format!("loadCodeAssist {status}: {text}"));
    }
    ...
    if needs_user_project && env_project().is_none() && existing_project.is_none() {
        return Err("Gemini Code Assist standard tier requires a GCP project. \
...
async fn onboard_user(
    ...
    if !status.is_success() {
        return Err(format!("onboardUser {status}: {text}"));
    }
    let lro: Value = serde_json::from_str(&text).map_err(|e| e.to_string())?;
    let op_name = lro
        .get("name")
        .and_then(Value::as_str)
        .ok_or_else(|| "onboardUser: missing operation name".to_owned())?
        .to_owned();
    ...
    for i in 0..60u32 {
        ...
        current = serde_json::from_str(&poll_text).map_err(|e| e.to_string())?;
    }
    let project = current
        .get("response")
    ...
    project.ok_or_else(|| "onboardUser: could not resolve project id".to_owned())
}
